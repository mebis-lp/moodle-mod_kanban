...
class helper {
     ...
     * @param mixed $users The users to send the notification to (can be int or array of int)
     ...
     */
    public static function send_notification(\cm_info $cm, string $messagename, mixed $users, object $data, string $altmessagename = null) {
        global $OUTPUT, $USER;
        if (is_int($users)) {
            $users = [$users];
        }
        ...
        $message->contexturlname = get_string('toboard', 'mod_kanban');
